...
Browser release **0.8.0 is experimental**. Firefox and Mullvad are outside this
...
| ---------------------- | ------------------------------------------------------- | ---------------------- |
| VS Code Marketplace    | `aic-notes-51.1.0.vsix`                                 | AIC Notes 51.1.0       |
| Chrome Web Store       | `dist-browser/artifacts/aic-browser-chromium-0.8.0.zip` | AIC — Page notes 0.8.0 |
| Microsoft Edge Add-ons | **the same** Chromium ZIP                               | AIC — Page notes 0.8.0 |

These are **expected names**, not proof that a file is ready. Standard Notes AIC
**43.1.0** and shared core **7.2.0** are coordinated with this release, but
Standard Notes is not submitted to these stores. AIC Notes **51.1.0** and browser
**0.8.0** keep credential rows compact on narrow panels: text-width labels,
flexible values, lock-and-six-dot protected copy, one trailing `+` creation menu,
and quiet row dividers. They also use direct Mermaid Markdown editing with a live
preview, one shared Edit button, and preview-only zoom. Quote accents, the short centered thematic break, and the blank text row
...
1. Take the **verified** `aic-notes-51.1.0.vsix` and checksum from
...
4. After publication, verify version **51.1.0** on `ldzyha.aic-notes` and install
...
4. Select **Submit for Review**. After approval, verify **0.8.0** and install it
...
4. Submit for review. When the status becomes **In the store**, verify **0.8.0**
...
**Статус:** це план першого ручного подання, а не повідомлення про публікацію. Зараз канал випуску — GitHub; у VS Code Marketplace, Chrome Web Store та Microsoft Edge Add-ons подання ще не підтверджені. Браузерний випуск **0.8.0 експериментальний**. Firefox і Mullvad не входять у цей план. Рекомендація: спершу завершити перевірки та матеріали, потім подати кожен магазин окремо й перевірити встановлення після схвалення.

| Куди                   | Файл для подання                                        | Цільова версія         |
| ---------------------- | ------------------------------------------------------- | ---------------------- |
| VS Code Marketplace    | `aic-notes-51.1.0.vsix`                                 | AIC Notes 51.1.0       |
| Chrome Web Store       | `dist-browser/artifacts/aic-browser-chromium-0.8.0.zip` | AIC — Page notes 0.8.0 |
| Microsoft Edge Add-ons | **той самий** Chromium ZIP                              | AIC — Page notes 0.8.0 |

Це **очікувані назви**, не доказ готовності файлів. Standard Notes AIC **43.1.0** і спільне ядро **7.2.0** координуються з цим випуском, але Standard Notes не подається до цих магазинів. AIC Notes **51.1.0** та браузер **0.8.0** ущільнюють credential rows на вузьких панелях: label займає ширину тексту, values лишаються гнучкими, protected copy показує замок і шість крапок, одне меню `+` продовжує останнє значення, а записи розділяє легка лінія. Mermaid редагується як Markdown-код із живим прев’ю, однаковою кнопкою Edit і масштабуванням лише у прев’ю. Акценти цитат, коротка центрована горизонтальна лінія та порожній текстовий рядок замість шаблону Account з’явилися у попередніх версіях. Попередній випуск браузера 0.4.0 додав Global Shared у зашифрованому сховищі поточного профілю. Це не синхронізація між застосунками й не реалізація майбутньої архітектури ключів або VS Code `global.aic`.
...
1. Візьміть **перевірений** `aic-notes-51.1.0.vsix` і його контрольну суму з [AIC Notes Releases](https://github.com/ldzyha/aic-notes/releases).
...
4. Натисніть **Submit for Review**. Після схвалення звірте версію **0.8.0** і встановіть із магазину в чистому профілі Chrome. Завантаження файлу саме по собі ще не означає публікації. [Офіційна інструкція](https://developer.chrome.com/docs/webstore/publish/).
...
4. Надішліть на перевірку. Коли статус стане **In the store**, звірте версію **0.8.0** і встановіть із Edge Add-ons у чистому профілі. Подання до Chrome **не** публікує продукт в Edge. [Офіційна інструкція](https://learn.microsoft.com/en-us/microsoft-edge/extensions/publish/publish-extension).
